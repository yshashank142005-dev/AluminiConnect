...
exports.sendOtp = async (email) => {
  ...
  if (transporter) {
    transporter
      ...
      .catch((error) => console.warn('OTP email delivery failed, fallback to server console:', error.message || error));
    return { delivered: true };
  }

  console.warn(`Email service not configured. OTP for ${email}: ${otp}`);
